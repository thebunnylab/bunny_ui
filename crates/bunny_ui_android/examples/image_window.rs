//! Images on the phone: the platform's own decoder reads a PNG this
//! example writes by hand (zlib with stored blocks needs no
//! compressor), and the same pixels ride the Vulkan atlas — small ones
//! as shared tiles, the big hero on a dedicated texture.
//!
//! ```sh
//! crates/bunny_ui_android/android/run-emu.sh image_window_android
//! ```
...
fn main() {
    ...
    // the label stands over its picture: a phone is narrow
    let row = |label: &str, view: Erased| {
        vstack!(text(label).monospaced().foreground_color(theme::fg_secondary()), view)
            .spacing(6.0)
    ...
    let root = vstack!(
        ...
        row("dedicated 300×180", erased(image(hero).resizable().frame(300.0, 180.0))),
        row(
            "icons 16 and 32",
            erased(hstack!(
    ...
    bunny_ui_android::run_window("bunny_ui — images", Size { width: 360.0, height: 800.0 }, root);
}
