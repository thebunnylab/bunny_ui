//! The frame harness with the PLATFORM text engine (FreeType) — the twin
//! of bunny-ui's `bench_pipeline` with real shaping, for a fair
//! comparison against any baseline that also shapes real text.
//!
//! ```sh
//! cargo run --release -p bunny-ui-linux --example bench_pipeline_linux
//! ```
...
fn main() {
    ...
    let engine = std::rc::Rc::new(FreeTypeEngine::new());
    ...
    use bunny_ui_linux::OffscreenGl;
    if let Some(mut gpu) = OffscreenGl::new(1520, 1280) {
    ...
    println!("\nfixture: finder 30 rows; viewport 760×640; FreeType text (real shaping)");
    println!(
