//! Images on linux: the shell's own decoder reads a PNG this example
//! writes by hand (zlib with stored blocks needs no compressor), and
//! the same pixels ride the GPU atlas — small ones as shared tiles,
//! the big hero on a dedicated texture.
//!
//! ```sh
//! cargo run -p bunny-ui-linux --example image_window_linux
//! ```
