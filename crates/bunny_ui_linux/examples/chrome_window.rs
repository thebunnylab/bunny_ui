//! The scene draws its own window chrome: no system title bar, the
//! scene's OWN caption buttons at the corner, and a bar that belongs
//! to the app — tabs, actions, identity. Drag the window by the bar
//! (buttons still click); the caption buttons are semantic marks, so
//! the platform closes, minimizes, maximizes — and hovers the snap
//! flyout over the maximize glyph, the system's own manner.
//!
//! ```sh
//! cargo run -p bunny-ui-linux --example chrome_window_linux
//! ```
...
use bunny_ui::layout::{Size, WindowControl};
use bunny_ui::prelude::*;

const BAR_H: f64 = 44.0;
/// Each caption button's width — the platform's own proportion.
const CAPTION_W: f64 = 46.0;
...
/// One of the window's own buttons: a small glyph, a hover wash, and
/// the semantic mark that makes the platform treat it as the real
/// thing. The empty click is the hover subscription — hover lands on
/// interactive targets, and the platform (not the click) activates
/// the button through the non-client road.
fn caption_button(
...
/// The maximize glyph: the platform's thin square, drawn by the house.
fn maximize_glyph() -> impl UnaryView {
    canvas(|ctx, painter| {
...
impl Component for App {
    fn body(self, _ctx: &Context) -> impl View {
        ...
        // where nothing interactive wins); the caption buttons answer
        // as the window's own.
        let bar = hstack!(
            text("bunny").bold().padding_edge(Edge::Leading, 14.0),
            spacer(),
            tabs,
            spacer(),
            text("de")
            ...
            caption_button(maximize_glyph(), WindowControl::Maximize, theme::row_hover()),
            caption_button(
                icon(symbol::CLOSE)
                    .font_size(10.0)
...
fn main() {
    ...
    bunny_ui_linux::run_window_chrome(
        "bunny — scene chrome",
        Size { width: 860.0, height: 560.0 },
        bunny_ui_linux::Chrome::Scene,
        runtime,
        App { tab: State::new(0) },
    );
}
