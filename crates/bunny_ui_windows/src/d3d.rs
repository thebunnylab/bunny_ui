...
float rect_sdf(float2 p, float4 rect, float radius) {
...
float rect_cov(float2 p, float4 rect, float radius) {
...
impl D3dStack {
    ...
    unsafe fn encode_frame(
        ...
        unsafe {
            ...
            (vtbl.clear_render_target_view)(context, target, clear.as_ptr());
            if runs.is_empty() {
                return;
            }
...
/// instance: the 64-byte rect wire and the 48-byte sprite wire stay
/// untouched. `radius == 0` is the straight rectangle every clip has
...
struct RoundClip {
    ...
    box4: [f32; 4],
    radius: f32,
    /// A cbuffer rounds to 16-byte registers; these three say so out
