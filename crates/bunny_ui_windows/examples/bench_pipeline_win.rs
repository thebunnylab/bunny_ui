//! The frame harness with the PLATFORM text engine (DirectWrite) — the twin
//! of bunny-ui's `bench_pipeline` with real shaping, for a fair
//! comparison against any baseline that also shapes real text.
//!
//! ```sh
//! cargo run --release -p bunny-ui-windows --example bench_pipeline_win
//! ```
...
use bunny_ui_windows::DirectWriteEngine;
...
fn main() {
    ...
    let engine = std::rc::Rc::new(DirectWriteEngine::new());
    ...
    // the second backend: the SAME display list presented by d3d11 —
    // walk + upload + encode + submit + the GPU itself, WAITED, so the
    // number hides nothing. the twin of the cpu paint row above.
    use bunny_ui_windows::OffscreenD3d;
    if let Some(mut gpu) = OffscreenD3d::new(1520, 1280) {
    ...
    if let Some(mut gpu) = OffscreenD3d::new(6048, 3928) {
    ...
    println!("\nfixture: finder 30 rows; viewport 760×640; DirectWrite text (real shaping)");
    println!(
        "gpu rows: walk + upload + encode + submit + WAIT on an offscreen target — \
