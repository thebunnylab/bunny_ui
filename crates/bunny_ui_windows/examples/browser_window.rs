...
//! places the box, and WebView2 composites above the scene inside
//! it — its own renderer, its own scroll, its own input, zero bytes
//! of bundled engine (the Evergreen runtime is found by registry).
//! The instrumentation floor rides along: a user script at document
//! start, the message bus, navigation reports and eval with the
//! value coming back.
//!
//! Run: `cargo run -p bunny-ui-windows --example browser_window`
//!
...
use bunny_ui_windows::{DirectWriteEngine, WicImageEngine};
#[cfg(target_os = "windows")]
use std::rc::Rc;
...
fn main() {
    ...
    bunny_ui_windows::run_window_with(
        "browser",
