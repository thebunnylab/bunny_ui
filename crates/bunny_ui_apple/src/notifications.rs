//! The notifications every Apple platform shows the same way —
//! UserNotifications — answering the notification doors of
//! `bunny_ui::app`: `notify` posts one, and the center's delegate
//! reports the one the person acted on as `AppEvent::NotificationActivated`.
//!
...
//! by name instead: a dev binary on the Mac is shown by wrapping it in
//! an `.app` whose `Info.plist` names a `CFBundleIdentifier`. The
//! phone is always a bundle.
//!
//! The delegate is the SHELL's own object — the application delegate
//! each shell already registers. This module never allocates a class:
//! it teaches a class still being born the center's doors
//! ([`add_delegate_methods`]), and is told which instance answers
//! ([`install_delegate`]) — at boot, before the launch returns, so a
//! tap on a notification that is the very thing launching the process
//! finds the door already open.
//!
//! The system asks the person once. A notification posted before they
//! answer is HELD here and posted the moment they allow it — the
//! center refuses a request it has no authorization for, quietly, and
//! `notify` promised never to be quiet.
...
use crate::ffi::{Id, Sel, class, class_addMethod, class_addProtocol, objc_getProtocol, sel};
...
unsafe extern "C" {
    ...
    #[link_name = "objc_msgSend"]
    fn msg_void_sel_id_bool(obj: Id, sel: Sel, a: Sel, b: Id, c: i8);
}
...
/// The object that answers the center — the shell's delegate, recorded
/// by [`install_delegate`]. A static, not a thread-local: the
/// authorization's answer arrives on a thread of the system's choosing
/// and needs the delegate to knock on the main thread's door.
static DELEGATE: AtomicPtr<c_void> = AtomicPtr::new(null_mut());
...
/// Notifications posted before the person answered: held, and posted
/// the moment they allow — or dropped, with the denial recorded for
/// the next post to report.
static PENDING: Mutex<Vec<Notification>> = Mutex::new(Vec::new());
...
/// The selector the authorization's answer knocks with, on the main
/// thread — the held notifications are posted from there.
const REPOST: &str = "bunnyRepostNotifications:";

/// Teaches `class` — still being born, before `objc_registerClassPair`
/// — the center's two doors and the repost, and makes it conform to
/// `UNUserNotificationCenterDelegate`. The class stays the shell's: the
/// Mac's `NSObject` delegate and the phone's `UIResponder` delegate
/// both learn the same three methods.
///
/// # Safety
///
/// `class` is a class pair the caller allocated and has not registered.
pub unsafe fn add_delegate_methods(class: Id) {
    let methods: [(&str, *const c_void, &str); 3] = [
        (
        ...
        (REPOST, bridge_repost as *const c_void, "v@:@"),
    ];
    unsafe {
        for (selector, imp, types) in methods {
            let types = CString::new(types).expect("type encoding");
            class_addMethod(class, sel(selector), imp, types.as_ptr());
        }
        let protocol = CString::new("UNUserNotificationCenterDelegate").expect("protocol name");
...
/// Hands the current center its delegate — an instance of a class that
/// went through [`add_delegate_methods`] — and remembers it for every
/// later post. Called at boot, before the launch returns: a tap on a
/// notification can be the very thing that launched the process, and
/// the response arrives early. A process with no bundle has no center,
/// and nothing happens.
///
/// # Safety
///
/// `delegate` is an object that lives as long as the process.
pub unsafe fn install_delegate(delegate: Id) {
    DELEGATE.store(delegate.cast(), Ordering::SeqCst);
    if bundled() {
        unsafe {
...
/// holds the notification until they answer; one that arrives while
/// the ask is open waits with it. The same `id` REPLACES, so a held
/// notification posted late is one notification, not two.
pub fn notify(notification: &Notification) -> Result<(), String> {
    ...
    unsafe {
        ...
        // whatever was held before the answer goes first, in order
        for held in take_held() {
            post(center, &held);
        }
...
fn hold(notification: &Notification) {
    PENDING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(notification.clone());
}

fn take_held() -> Vec<Notification> {
    std::mem::take(&mut *PENDING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()))
}

/// One notification, to the center: the category for its buttons,
/// the content, the request under its own id.
unsafe fn post(center: Id, notification: &Notification) {
    unsafe {
        let delegate = DELEGATE.load(Ordering::SeqCst);
        if !delegate.is_null() {
            msg_void_id(center, sel("setDelegate:"), delegate.cast());
        }
...
thread_local! {
    ...
    /// handed each time, whole. A thread-local like the notifier's
    /// own slot: a post only ever runs on the app's thread.
...
/// The person answered — on a thread of the system's choosing. Granted:
/// the delegate is knocked on the main thread to post what was held.
/// Denied: the held notifications are dropped, and the reason kept.
...
/// `bunnyRepostNotifications:` — on the main thread, once the person
/// allowed: what was held goes to the center, in order.
extern "C" fn bridge_repost(_this: Id, _sel: Sel, _object: Id) {
    unsafe {
        let center = msg_id(class("UNUserNotificationCenter"), sel("currentNotificationCenter"));
        if center.is_null() {
            return;
