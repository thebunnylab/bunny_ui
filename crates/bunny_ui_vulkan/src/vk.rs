//! Vulkan presentation — the tier the Linux and Android shells share;
//! each shell keeps its own ladder around it (on Linux, by the user's
//! own decision: vk first, gl when it cannot come up, cpu last).
//!
//! House rules apply: no dependencies. The loader comes in through
//! `dlopen` of the system's `libvulkan.so.1` (`libvulkan.so` on
//! Android); every entry point
...
//!   do not exist here.
//!
//! WSI speaks three doors natively (`wl_surface`, xcb window and
//! `ANativeWindow`) — no `wl_egl_window` anywhere on this tier. The
//! shell hands its window in as a [`SurfaceSource`]; a lost surface or
//! a lost device comes back by name in [`Presented`], and the shell
//! decides what catches it (a new window, a rebuild, the next tier).
...
const ST_ANDROID_SURFACE_CREATE_KHR: u32 = 1000008000;
...
// VkSurfaceTransformFlagBitsKHR: the one transform the tier asks for
const TRANSFORM_IDENTITY: u32 = 0x1;
...
struct WsiFns {
    ...
    >,
    create_android_surface: Option<
        unsafe extern "C" fn(
            Instance,
            *const AndroidSurfaceCreateInfo,
            *const c_void,
            *mut SurfaceKHR,
        ) -> VkResult,
    >,
...
fn resolve_wsi(gipa: GipaFn, instance: Instance) -> Option<WsiFns> {
    ...
    Some(WsiFns {
        ...
        create_android_surface: {
            let address = unsafe { gipa(instance, c"vkCreateAndroidSurfaceKHR".as_ptr()) };
            (!address.is_null()).then(|| unsafe { std::mem::transmute(address) })
        },
...
fn loader() -> Option<(GipaFn, CreateInstanceFn)> {
    ...
    *LOADED.get_or_init(|| {
        // the desktop's soname first; Android ships the bare name
        let lib = [c"libvulkan.so.1", c"libvulkan.so"]
            .into_iter()
            .map(|name| unsafe { dlopen(name.as_ptr(), RTLD_NOW) })
            .find(|lib| !lib.is_null())?;
...
/// The native window a swapchain wraps — one variant per door. The
/// shell owns the window; the tier reads it at surface creation only.
#[derive(Clone, Copy, Debug)]
pub enum SurfaceSource {
    /// An xcb window on its connection.
    Xcb { connection: *mut c_void, window: u32 },
    /// A `wl_surface` on its display.
    Wayland { display: *mut c_void, surface: *mut c_void },
    /// An `ANativeWindow`.
    Android { window: *mut c_void },
}

/// The WSI door a window stands in front of — names the instance
/// extension the door needs.
...
impl Door {
    fn of(source: SurfaceSource) -> Door {
        match source {
            SurfaceSource::Xcb { .. } => Door::Xcb,
            SurfaceSource::Wayland { .. } => Door::Wayland,
            SurfaceSource::Android { .. } => Door::Android,
        }
    }

    fn extension(self) -> &'static CStr {
        match self {
            Door::Xcb => c"VK_KHR_xcb_surface",
            Door::Wayland => c"VK_KHR_wayland_surface",
            Door::Android => c"VK_KHR_android_surface",
        }
    }
}

/// Which target the stack will feed — decides extensions and the
...
struct VkStack {
    ...
    surface: SurfaceKHR,
    /// `maxImageDimension2D`: the ceiling a shell's window keeps under.
    max_image_2d: u32,
    instance: Instance,
...
impl VkStack {
    fn create(target: VkTarget, source: Option<SurfaceSource>) -> Option<VkStack> {
        let result = Self::build(target, source);
    ...
    fn build(target: VkTarget, source: Option<SurfaceSource>) -> Result<VkStack, String> {
        let (gipa, create_instance) = loader().ok_or("no libvulkan on this system")?;
        unsafe {
            ...
            match target {
                VkTarget::Window(door) => {
                    extensions.push(c"VK_KHR_surface".as_ptr());
                    extensions.push(door.extension().as_ptr());
                }
                VkTarget::Offscreen => {}
            }
            ...
            // the window's surface comes before the device: the render
            // pass wears the format the surface offers, and only the
            // surface can say which
            let surface = match (wsi.as_ref(), source) {
                (Some(wsi), Some(source)) => match create_surface_raw(instance, wsi, source) {
                    Ok(surface) => surface,
                    Err(reason) => {
            ...
            if count == 0 {
                destroy_surface_raw(wsi.as_ref(), instance, surface);
                (fns.destroy_instance)(instance, std::ptr::null());
            ...
            // the texture ceiling (`maxImageDimension2D`, the second
            // word of the limits block), and the format law: a window
            // wears the surface's first offer of the two the tier
            // speaks; an offscreen target renders RGBA8 by decree
            let max_image_2d = {
                let mut properties = std::mem::zeroed::<PhysicalDeviceProperties>();
            ...
            let format = match wsi.as_ref() {
                Some(wsi) if surface != 0 => {
                    let Some(format) = pick_surface_format(wsi, physical, surface) else {
                        destroy_surface_raw(Some(wsi), instance, surface);
                        (fns.destroy_instance)(instance, std::ptr::null());
                        return Err("no B8G8R8A8_UNORM or R8G8B8A8_UNORM surface format".into());
                    };
                    format
                }
                _ => FORMAT_R8G8B8A8_UNORM,
            };
            let priority = 1.0f32;
            ...
            let final_layout = match target {
                VkTarget::Offscreen => IMAGE_LAYOUT_TRANSFER_SRC,
                VkTarget::Window(_) => IMAGE_LAYOUT_PRESENT_SRC_KHR,
            };
...
impl Drop for VkStack {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            destroy_surface_raw(self.wsi.as_ref(), self.instance, self.surface);
...
fn build_swapchain(
    ...
    unsafe {
        ...
        // the format law: the stack's own format (BGRA8 or RGBA8 UNORM,
        // picked at build), NEVER sRGB — a surface that stopped offering
        // it refuses the swapchain
        if surface_formats(wsi, stack.physical, surface)
            .iter()
            .all(|format| format.format != stack.format)
        {
            eprintln!("bunny_ui vk: the surface does not offer the stack's format");
            return None;
        }
        ...
        let info = SwapchainCreateInfo {
            ...
            family_count: 0,
            families: std::ptr::null(),
            // identity where the surface allows it: the fragment shaders
            // judge coverage in device space, so a rotated phone is
            // rotated by the compositor, never by the tier
            pre_transform: if capabilities.supported_transforms & TRANSFORM_IDENTITY != 0 {
                TRANSFORM_IDENTITY
            } else {
        ...
        for &image in &images {
            let view = stack.view_of(image, stack.format)?;
...
/// The window presenter: one stack, one surface at a time, three
/// frames in flight. The surface lives in the stack and can be let go
/// and taken up again — the phone's window comes and goes — while the
/// device, the pipelines, the atlas and its textures stay.
pub struct VkPresenter {
    stack: VkStack,
    /// `None` while detached — a present then paints nothing.
...
impl VkPresenter {
    ...
    fn ensure_glass(&mut self) {
        ...
        if let Some(targets) = &self.glass {
            if targets.extent == extent {
                return;
            }
            self.stack.release_glass(targets);
        }
        self.glass = self.stack.glass_targets(extent);
...
/// What a present came back with. `Ok` covers a frame that painted and
/// a frame with nothing to paint; the two losses are the shell's to
/// answer — a surface by a new window, a device by a new presenter or
/// by the next tier down.
...
impl VkPresenter {
    /// Presents one frame. `pre(scale)` runs after the queue submit and
    /// before the WSI present — `false` keeps the frame for the next
    /// redraw (a Wayland surface not yet configured) and answers `Ok`;
    /// `post()` runs after a present the WSI accepted. Detached, with no
    /// swapchain, the frame paints nothing and answers `Ok`.
    #[allow(clippy::too_many_arguments)]
    pub fn present(
        &mut self,
        display: &DisplayList,
        ...
        for attempt in 0..2 {
            ...
            let handle = self.swapchain.as_ref().map_or(0, |swapchain| swapchain.handle);
            let acquired = unsafe {
            ...
            match acquired {
                ...
                VK_ERROR_OUT_OF_DATE_KHR if attempt == 0 => {
                    if !self.recreate_swapchain(physical) {
                        return Presented::DeviceLost;
                    }
                }
                VK_ERROR_SURFACE_LOST_KHR => return Presented::SurfaceLost,
        ...
        self.ensure_glass();
        let swapchain = self.swapchain.as_ref().expect("the image was acquired from a swapchain");
        record_frame(
            &self.stack,
            &mut self.ground,
            &self.slots[index],
        ...
        unsafe {
            ...
            match presented {
                ...
                VK_ERROR_OUT_OF_DATE_KHR => {
                    // accepted but stale — the next frame rebuilds
                    let _ = self.recreate_swapchain(physical);
                }
                VK_SUBOPTIMAL_KHR => {
                    // a rotated phone under an identity transform says
                    // this every frame; only a moved extent means it
    ...
    fn recreate_swapchain(&mut self, wanted: (usize, usize)) -> bool {
        ...
        let old = self.swapchain.as_ref().map_or(0, |swapchain| swapchain.handle);
        ...
        if let Some(mut stale) = self.swapchain.take() {
            destroy_swapchain(&self.stack, &mut stale);
        }
    ...
    fn extent_stale(&self) -> bool {
        ...
        let asked = unsafe {
            (wsi.surface_capabilities)(self.stack.physical, self.stack.surface, &mut capabilities)
        };
        ok(asked)
            && capabilities.current_extent[0] != u32::MAX
            && (capabilities.current_extent[0], capabilities.current_extent[1])
                != swapchain.extent
    }
}
...
/// One surface over the shell's window — the door's own creator.
fn create_surface_raw(
    ...
    let made = unsafe {
        match source {
            SurfaceSource::Wayland { display, surface: wl_surface } => {
                ...
                let create = wsi.create_wayland_surface.ok_or("no wayland WSI")?;
                create(instance, &info, std::ptr::null(), &mut surface)
            }
            SurfaceSource::Xcb { connection, window } => {
                ...
                let create = wsi.create_xcb_surface.ok_or("no xcb WSI")?;
                create(instance, &info, std::ptr::null(), &mut surface)
            }
            SurfaceSource::Android { window } => {
                let info = AndroidSurfaceCreateInfo {
                    s_type: ST_ANDROID_SURFACE_CREATE_KHR,
                    p_next: std::ptr::null(),
                    flags: 0,
                    window,
                };
                let create = wsi.create_android_surface.ok_or("no android WSI")?;
                create(instance, &info, std::ptr::null(), &mut surface)
            }
        }
    };
    if !ok(made) {
        return Err("no WSI surface".into());
    }
    Ok(surface)
}

fn destroy_surface_raw(wsi: Option<&WsiFns>, instance: Instance, surface: SurfaceKHR) {
    if surface == 0 {
        return;
    }
    if let Some(wsi) = wsi {
        unsafe { (wsi.destroy_surface)(instance, surface, std::ptr::null()) };
    }
}
...
fn surface_formats(
    ...
    unsafe {
        let mut count = 0;
        (wsi.surface_formats)(physical, surface, &mut count, std::ptr::null_mut());
        let mut formats = vec![SurfaceFormat::default(); count.max(1) as usize];
        (wsi.surface_formats)(physical, surface, &mut count, formats.as_mut_ptr());
        formats.truncate(count as usize);
        formats
    }
}

/// The format law: `B8G8R8A8_UNORM` where the surface offers it (the
/// desktop's habit), else `R8G8B8A8_UNORM` (the phone's), and NEVER
/// sRGB — the instance colours are straight RGBA bytes, and the
/// attachment's format alone decides their order.
fn pick_surface_format(
    wsi: &WsiFns,
    physical: PhysicalDevice,
    surface: SurfaceKHR,
) -> Option<u32> {
    let offered = surface_formats(wsi, physical, surface);
    [FORMAT_B8G8R8A8_UNORM, FORMAT_R8G8B8A8_UNORM]
        .into_iter()
        .find(|wanted| offered.iter().any(|format| format.format == *wanted))
}

impl VkPresenter {
    /// Instance, device, pipelines, the surface over `source` and a
    /// swapchain at `size` (device pixels). `None` = the tier refuses,
    /// and one line on stderr says why. `scene` = the window is a scene
    /// of the shell's own chrome, with the 8-point corner mask.
    pub fn install(source: SurfaceSource, size: (u32, u32), scene: bool) -> Option<VkPresenter> {
        let stack = VkStack::create(VkTarget::Window(Door::of(source)), Some(source))?;
        let Some(swapchain) = build_swapchain(&stack, stack.surface, size, 0) else {
    ...
    /// The window changed size under the swapchain — the phone tells
    /// the shell, and Vulkan may never say `OUT_OF_DATE` for it. `false`
    /// = nothing to do (detached, or the same size already).
    pub fn resize(&mut self, size: (u32, u32)) -> bool {
        match self.swapchain.as_ref() {
            Some(swapchain) if swapchain.extent != size => {}
            _ => return false,
        }
        self.recreate_swapchain((size.0 as usize, size.1 as usize))
    }

    /// Forgets the retained frame, so the NEXT present cannot skip —
    /// the ack road's skip-breaker, same contract as the gl tier's.
    pub fn invalidate(&mut self) {
        self.retained = None;
    }

    /// The window is going away: waits for the device, destroys the
    /// swapchain and the surface. The device, the pipelines, the atlas
    /// and every texture stay for the window that comes next.
    pub fn detach_surface(&mut self) {
        ...
        if let Some(mut swapchain) = self.swapchain.take() {
            destroy_swapchain(&self.stack, &mut swapchain);
        }
        destroy_surface_raw(self.stack.wsi.as_ref(), self.stack.instance, self.stack.surface);
        self.stack.surface = 0;
        self.retained = None;
    }

    /// A new window on the same device — the phone's, back from the
    /// background. `false` = the surface or its swapchain refused, and
    /// the presenter stays detached.
    pub fn attach_surface(&mut self, source: SurfaceSource, size: (u32, u32)) -> bool {
        if self.stack.surface != 0 {
            self.detach_surface();
        }
        let Some(wsi) = self.stack.wsi.as_ref() else { return false };
        let surface = match create_surface_raw(self.stack.instance, wsi, source) {
            Ok(surface) => surface,
            Err(reason) => {
                eprintln!("bunny_ui vk: {reason}");
                return false;
            }
        };
        // the format was fixed at build; a surface that does not offer
        // it refuses inside build_swapchain
        self.stack.surface = surface;
        let Some(swapchain) = build_swapchain(&self.stack, surface, size, 0) else {
            destroy_surface_raw(self.stack.wsi.as_ref(), self.stack.instance, surface);
            self.stack.surface = 0;
            return false;
        };
    ...
    /// Whether a surface and its swapchain stand under the presenter.
    pub fn is_attached(&self) -> bool {
        self.stack.surface != 0 && self.swapchain.is_some()
    }

    /// `maxImageDimension2D` — the ceiling a shell's window must keep
    /// under (a texture and a swapchain image share it).
    pub fn max_texture_size(&self) -> u32 {
        self.stack.max_image_2d
    }
}

impl Drop for VkPresenter {
    fn drop(&mut self) {
        // the swapchain, then the surface, then the stack lets the
        // device and the instance go; slots and ground images die with
        // the device
        self.detach_surface();
    }
}
...
impl OffscreenVk {
    pub fn new(width: usize, height: usize) -> Option<OffscreenVk> {
        ...
        let stack = VkStack::create(VkTarget::Offscreen, None)?;
